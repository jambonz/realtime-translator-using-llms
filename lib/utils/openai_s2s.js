...
const safeClose = require('./safe-close');
const fs = require('fs');
...
class Adaptor extends Emitter {
  ...
  safeClose() {
    if (this.isClosed) {
      this.logger.info('Adapter already closed, ignoring duplicate close request');
      return;
    }
    
    this.isClosed = true;
    this.logger.info('Safely closing OpenAI connection');
    ...
    if (this.ws) {
      try {
        ...
        if (!this.connectionEstablished) {
          ...
          // We don't call ws.close() here - this is crucial
          this.ws = null;
          return;
        }
