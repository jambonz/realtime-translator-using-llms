...
class Switchman {
  constructor(logger, call_sid) {
    ...
    if (process.env.OPENAI_API_KEY) {
      this.a_adapter = new OpenAIAdapter(logger, process.env.OPENAI_API_KEY, AI_PROMPT_CALLING_PARTY);
      this.b_adapter = new OpenAIAdapter(logger, process.env.OPENAI_API_KEY, AI_PROMPT_CALLED_PARTY);
    }
    else {
